...
pA <- ggplot(detected_df, aes(x = ibs_status, y = detected_pathways, fill = ibs_status)) +
  ...
  labs(
    title = "HUMAnN pathway reconstruction",
...
save_plot(pA, "fig7A_detected_pathways_by_group_no latters", 5.2, 4.2)
save_plot(pB, "fig7B_top_metacyc_pathways_heatmap_no latters", 6.2, 4.5)
save_plot(pC, "fig7C_metaphlan_species_pcoa_no lattters", 5.4, 4.5)
save_plot(pD, "fig7D_sq_pathway_detection_summary_no latters", 6.2, 4.1)

# -----------------------------
# Save combined 2x2 figure
# -----------------------------

combined_pdf <- file.path(out_dir, "fig7_combined_functional_taxonomic_profile_ NO_LATTERS.pdf")
combined_png <- file.path(out_dir, "fig7_combined_functional_taxonomic_profile_NO_LATTERS.png")
